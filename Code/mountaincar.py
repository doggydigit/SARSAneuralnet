...
class MountainCar():
    ...
    def __init__(self, g = 10.0, d = 100.0, H = 10., m = 10.0, 
                force_amplitude = 3.0, reward_amplitude = 1., 
                 reward_threshold = 0.0, nx = 4, ny = 4):
        ...
        self.nx = nx
        self.ny = ny
        self.network = sarsa.Network(nx,ny)
        self.updatequiver = True
    ...
    def chose_action(self):
        return self.network.GetAction(self.x,self.x_d)
    ...
    def simulate_timesteps(self, n = 100, dt = 0.01):
    ...
    def learn(self, action):
		self.network.UpdateWeights(action,self.R)
...
class MountainCarViewer():
    ...
    def create_figure(self, n_steps, max_time, f = None):
        ...
        # replace force plot with weight plot
        self.ax_forces = plb.subplot(2,2,3)
        self.h_forces = self._plot_weights()
        plb.axis(xmin = -150, xmax = 30, 
                 ymin = -15,
                 ymax = 15)
    ...
    def update_figure(self):
        ...
        """
        #if(self.mountain_car.updatequiver):
			#self.f.clear()
			#self.create_figure(2000,200,self.f)
        ...
        #self.h_forces.set_color("red")
        #self.h_forces.set_UVC(zeros((self.nx,self.ny)))
        #self.f.canvas.draw()
        self._plot_weights(self.h_forces)
    ...
    def _plot_weights(self, handle = None):
		"""plot the force applied by the car vs time.
		"""
		# create the plots
		if handle is None:
			u = np.zeros((self.mountain_car.nx,self.mountain_car.ny))
			v = np.zeros((self.mountain_car.nx,self.mountain_car.ny))
			handle = plb.quiver(
				np.linspace(-150,0,self.mountain_car.nx),
				np.linspace(-15,15,self.mountain_car.ny),
				u,v,scale=10
			)
			...
			if(True):
				...
				u = np.zeros((self.mountain_car.nx,self.mountain_car.ny))
				v = np.zeros((self.mountain_car.nx,self.mountain_car.ny))
				for i in range(self.mountain_car.nx):
					s = np.sign(i-self.mountain_car.ny/2)
					for j in range(self.mountain_car.ny):
						v[i][j] = self.mountain_car.network.inputlayer[i][j].GetMaxWeight()
						u[i][j] = s
				
				handle.set_UVC(u,v)
				#self.mountain_car.updatequiver = False
			return handle
